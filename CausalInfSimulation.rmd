...
estimate_NDE <- function(data, x = 1, x_star = 0) {
  ...
  # By monte carlo approximation, integral over s is approximated by sum over observed s values
  # p(s) is approximated by the empirical distribution of s
  p_s <- 1/n
  ...
  for (i in 1:n) {
    ...
    for(z in c(0,1)) {
      ...
      nde_sum <- nde_sum + (y_x - y_xstar)* p_z_xstar * p_s
    }
  }
...
estimate_NIE <- function(data, x = 1, x_star = 0) {
  ...
  for (i in 1:n) {
    ...
    for(z in c(0,1)) {
      ...
      nie_sum <- nie_sum + y_z_xstar * (p_zx - p_z_xstar) * p_s
    }
  }
